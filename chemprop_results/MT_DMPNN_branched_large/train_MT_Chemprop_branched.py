...
from branched import BranchedFFN

hp_dict = {
    "depth": 6,
    "message_hidden_dim": 1200,
    "trunk_n_layers": 2,
...
def standard_multitask_train(train_dset, val_dset, test_dset, targets, scaler, hp_dict):
    num_workers = 16
    ...
    ffn = BranchedFFN(
        task_groups=[[n] for n in range(len(targets))],
        n_tasks=len(targets),
        task_weights=torch.as_tensor(hp_dict["task_weights"]),
        output_transform=output_transform,
        input_dim=hp_dict["message_hidden_dim"],
        trunk_n_layers=hp_dict["trunk_n_layers"],
        trunk_hidden_dim=hp_dict["trunk_hidden_dim"],
...
trainer_, mpnn_, results_ = standard_multitask_train(
